...
/**
 * Sets up the environment to activate autocomplete. Returns two functions, one
 * hook that needs to be set up with the wanted CodeMirror instance, and a
 * callback that can be used to populate the autocomplete databases.
 *
 * @return  {any}  Two callbacks, autocompleteHook and setAutocompleteDatabase
 */
export default function autocomplete (): {
  autocompleteHook: (cm: CodeMirror.Editor) => void
  setAutocompleteDatabase: (type: string, database: any) => void
} {
  // We have to scope every variable, so that every editor instance has its own
  ...
  // This is a tad stupid, but since we have a circular dependency between the
  // snippets keymap and the nextTabstop function, we must define it here as a
  // dummy, call it inside the 'Tab'-handler below, and afterwards overwrite it
  // with the correct nextTabstop function
  let nextTabstop = (cm: CodeMirror.Editor): void => {}
  ...
  const snippetsKeymap: CodeMirror.KeyMap = {
    'Tab': (cm: CodeMirror.Editor) => { nextTabstop(cm) },
   ...
   */
  const collectHeadingIDs = function (cm: CodeMirror.Editor): void {
  ...
  const beginAutocomplete = function (cm: CodeMirror.Editor, autocompleteDatabase: keyof typeof availableDatabases): void {
    ...
    const DISALLOW_SPACES = [ 'tags', 'headings' ]
   ...
   */
  const shouldBeginAutocomplete = function (cm: CodeMirror.Editor): keyof typeof availableDatabases | undefined {
    ...
    const linkStartRange = cm.getRange(
      { line: cursor.line, ch: cursor.ch - linkStart.length },
    ...
    if (codeBlockRE.test(line)) {
      ...
      let modeLineBefore = cm.getModeAt({
        line: cursor.line - 1,
        ch: 0
      })
   ...
   */
  const getHints = function (term: string): any[] {
   ...
   */
  const hintFunction = function (cm: CodeMirror.Editor, opt: CodeMirror.ShowHintOptions): CodeMirror.Hints | undefined {
    ...
    on(completionObject, 'pick', (completion: any) => {
      ...
      if (currentDatabase === 'files') {
        ...
        if (citeStyle === 'regular' && noBrackets) {
          ...
          const toCh = autocompleteStart.ch + (completion.text as string).length
          cm.setSelection({ line: lineNo, ch: fromCh }, { line: lineNo, ch: toCh }, { scroll: false })
   ...
   */
  const getTabMarkers = function (cm: CodeMirror.Editor, from: CodeMirror.Position, to: CodeMirror.Position): TextSnippetTextMarker[] {
    ...
    const endMarker = cm.setBookmark(to, {
      widget: endMarkerElement
    })
      ...
      while ((match = varRE.exec(line)) !== null) {
        ...
        if (replaceWith !== undefined) {
          ...
          const marker = cm.markText(
            localFrom, {
              line: localFrom.line,
              ch: localFrom.ch + replaceWith.length
            }, {
              className: 'tabstop'
            }
          )
   ...
   */
  const replaceSnippetVariables = function (text: string, cm: CodeMirror.Editor): string {
   ...
   * @param   {CodeMirror.Editor}  cm  The editor instance
   */
  nextTabstop = function (cm: CodeMirror.Editor): void {
  ...
  // Now begin exporting (returning) the corresponding functions
  return {
    /**
